...
contract AgreementContract is Admin, IAgreementContract {
    ...
    mapping(uint256 => Skill[]) public agreementSkills;
    ...
    function createAgreement(
        ...
        for (uint256 i = 0; i < _skills.length; i++) {
            agreementSkills[nextAgreementId].push(Skill({name: _skills[i].name, level: _skills[i].level}));
        }
    ...
    function getAllAgreements() external view override returns (Agreement[] memory) {
        return agreements;
    }

    function getContractorAgreementIds(address _contractor) external view override returns (uint256[] memory) {
        return contractorAgreements[_contractor];
    }

    function getProfessionalAgreementIds(address _professional) external view override returns (uint256[] memory) {
        return professionalAgreements[_professional];
    }

    function getAgreementById(uint256 _id) external view override returns (Agreement memory) {
        require(_id > 0 && _id <= agreements.length, "Invalid agreement ID");
        return agreements[_id - 1];
    }

    function getSkillsByAgreementId(uint256 _agreementId) external view override returns (Skill[] memory) {
    ...
    function makePayment(uint256 _agreementId, uint256 _amountToPay, address _paymentAddress) external {
        ...
        require(
            token.transferFrom(msg.sender, address(this), _amountToPay), "User must approve the amount of the agreement"
        );
    ...
    function setFees(uint256 _feePercentage, uint256 _kyodoTreasuryFee, uint256 _communityDAOFee)
        external
