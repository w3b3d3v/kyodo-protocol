...
describe("agreement_program", () => {
  ...
  it("Initialized Agrement Id Counter", async () => {
    // Add your test here.
    const tx = await program.methods.initializeAgreementIdCounter().accounts({
      idCounter: idAddress.publicKey,
      //protocol wallet
      user: provider.wallet.publicKey,
      systemProgram: anchor.web3.SystemProgram.programId,
    }).signers([idAddress]).rpc();

    const fetchedAgreement = await program.account.agreementIdCounter.fetch(idAddress.publicKey);
    console.log("Your counter account publickey", idAddress.publicKey.toBase58(),);
    console.log("Your counter account", fetchedAgreement);
